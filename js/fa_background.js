  ...
  chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
      if (request.greeting == 'get_vars')
        sendResponse({
          'display_callout': (localStorage['display_callout'] != undefined) ? parseBoolean(localStorage['display_callout']) : default_display_callout,
          'display_icon': (localStorage['display_icon'] != undefined) ? parseBoolean(localStorage['display_icon']) : default_display_icon,
          'border_color': (localStorage['border_color'] != undefined) ? localStorage['border_color'] : default_border_color,
          'border_width': (localStorage['border_width'] != undefined) ? localStorage['border_width'] : default_border_width,
          'border_style': (localStorage['border_style'] != undefined) ? localStorage['border_style'] : default_border_style
        });
    }
  );
}
